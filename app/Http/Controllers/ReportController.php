...
class ReportController extends Controller
{
    ...
    public function downloadPdf(Request $request, Siswa $siswa)
    {
        $type = $request->input('type', 'UTS');
        $tahunAjaranId = $request->input('tahun_ajaran_id', session('tahun_ajaran_id'));
        ...
        try {
            ...
            // 1. Check if DOCX file already exists (from previous download)
            $existingDocxPath = $this->findExistingDocxFile($siswa, $type, $tahunAjaranId);
            
            if ($existingDocxPath && file_exists($existingDocxPath)) {
                \Log::info('Using existing DOCX file for PDF conversion', [
                    'path' => $existingDocxPath
                ]);
                $docxPath = $existingDocxPath;
            } else {
                // 2. Generate new DOCX file if not exists
                \Log::info('Generating new DOCX file for PDF conversion');
                ...
                if (!$result['success']) {
                    throw new \Exception($result['message'] ?? 'Gagal generate DOCX');
                }
                
                $docxPath = storage_path('app/public/' . $result['path']);
                
                // Save history if new file generated
                $this->saveGenerationHistory($siswa, $template, $type, $tahunAjaranId, $result['path']);
            }
            
            // 3. Convert DOCX to PDF
            $pdfPath = $this->convertDocxToPdf($docxPath, $siswa, $type);
            
            if (!$pdfPath || !file_exists($pdfPath)) {
                throw new \Exception('Gagal convert DOCX ke PDF');
            }
            
            // 4. Generate clean filename for download
            $cleanName = preg_replace('/[^\w\s]/gi', '', $siswa->nama);
            $cleanName = preg_replace('/\s+/', '_', $cleanName);
            $pdfFilename = "Rapor_{$type}_{$cleanName}.pdf";
            ...
            \Log::error('Error in downloadPdf:', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
                'siswa_id' => $siswa->id,
                'type' => $type
    ...
    /**
     * Convert DOCX to PDF using available libraries
     */
    protected function convertDocxToPdf($docxPath, Siswa $siswa, $type)
    {
        try {
            // Method 1: Convert via HTML (most reliable)
            return $this->convertDocxToPdfViaHtml($docxPath, $siswa, $type);
            
        } catch (\Exception $e) {
            \Log::warning('HTML conversion failed, trying direct conversion', [
                'error' => $e->getMessage()
            ]);
            
            try {
                // Method 2: Direct conversion (fallback)
                return $this->convertDocxToPdfDirect($docxPath, $siswa, $type);
            } catch (\Exception $e2) {
                \Log::error('All PDF conversion methods failed', [
                    'html_error' => $e->getMessage(),
                    'direct_error' => $e2->getMessage()
                ]);
                throw new \Exception('Gagal convert ke PDF: ' . $e2->getMessage());
            }
        }
    }
     ...
     */
    protected function cleanHtmlForPdf($htmlContent)
    {
        // Remove problematic elements and fix formatting for PDF
        $htmlContent = preg_replace('/<style[^>]*>.*?<\/style>/is', '', $htmlContent);
        
        // Add basic PDF-friendly CSS
        $pdfCss = '
        <style>
            body { 
        ...
        $htmlContent = preg_replace('/<head[^>]*>/i', '<head>' . $pdfCss, $htmlContent);
    ...
    /**
     * Direct DOCX to PDF conversion (fallback method)
     */
    protected function convertDocxToPdfDirect($docxPath, Siswa $siswa, $type)
    {
        // This is a simplified approach - in production you might want to use
        // a more robust solution like LibreOffice via command line
        
        $cleanName = preg_replace('/[^\w\s]/gi', '', $siswa->nama);
        $cleanName = preg_replace('/\s+/', '_', $cleanName);
        $pdfFilename = "Rapor_{$type}_{$cleanName}_" . time() . ".pdf";
        $pdfPath = storage_path('app/public/generated/' . $pdfFilename);
        
        // For now, we'll use a simple HTML-based conversion
        // In production, consider using:
        // - LibreOffice command line: soffice --headless --convert-to pdf
        // - pandoc
        // - or a commercial API like CloudConvert
        
        throw new \Exception('Direct conversion not implemented. Use HTML method.');
    }

    /**
     * Convert DOCX to PDF via HTML (most reliable method)
     */
    protected function convertDocxToPdfViaHtml($docxPath, Siswa $siswa, $type)
    {
        // 1. Load DOCX and convert to HTML
        $phpWord = IOFactory::load($docxPath);
        $htmlWriter = IOFactory::createWriter($phpWord, 'HTML');
        
        // Create temporary HTML file
        $tempHtmlPath = tempnam(sys_get_temp_dir(), 'rapor_') . '.html';
        $htmlWriter->save($tempHtmlPath);
        
        // 2. Read HTML content and clean it up
        ...
        \Log::info('Successfully converted DOCX to PDF via HTML', [
            'docx_path' => $docxPath,
            'pdf_path' => $pdfPath,
    ...
    /**
     * Find existing DOCX file from previous downloads
     */
    protected function findExistingDocxFile(Siswa $siswa, $type, $tahunAjaranId)
    {
        // Look for existing file in generation history
        $latestGeneration = \App\Models\ReportGeneration::where('siswa_id', $siswa->id)
            ->where('type', $type)
            ->where('tahun_ajaran_id', $tahunAjaranId)
            ->whereNotNull('generated_file')
            ->latest('generated_at')
            ->first();
        
        if ($latestGeneration && $latestGeneration->generated_file) {
            $fullPath = storage_path('app/public/' . $latestGeneration->generated_file);
            
            if (file_exists($fullPath)) {
                \Log::info('Found existing DOCX file from history', [
                    'path' => $fullPath,
                    'generated_at' => $latestGeneration->generated_at
                ]);
                return $fullPath;
            }
        }
        
        // Alternative: Look for files by naming pattern
        $cleanName = preg_replace('/[^\w\s]/gi', '', $siswa->nama);
        $cleanName = preg_replace('/\s+/', '_', $cleanName);
        $pattern = "Rapor_{$type}_{$cleanName}*.docx";
        $searchDir = storage_path('app/public/generated/');
        ...
        if (is_dir($searchDir)) {
            ...
            if (!empty($files)) {
                ...
                \Log::info('Found existing DOCX file by pattern', [
                    'path' => $files[0]
                ]);
                return $files[0];
            }
     ...
     */
    public function previewPdf(Request $request, Siswa $siswa)
    {
        $type = $request->input('type', 'UTS');
        $tahunAjaranId = $request->input('tahun_ajaran_id', session('tahun_ajaran_id'));
        
        try {
            // Similar logic as downloadPdf but return inline
            $existingDocxPath = $this->findExistingDocxFile($siswa, $type, $tahunAjaranId);
            ...
            if ($existingDocxPath && file_exists($existingDocxPath)) {
                ...
                if (!$template) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Template rapor tidak ditemukan'
                    ], 404);
                }
            ...
            if (!$pdfPath || !file_exists($pdfPath)) {
                throw new \Exception('Gagal convert ke PDF');
            }
